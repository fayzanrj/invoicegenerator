...
/**
 * Middleware to authorize access based on JWT access token.
...
const authorised = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!user || !user.userId) return ThrowUnAuthorizedError(res);

    // Finding user details by user ID from the token
    const userDetails = await User.findById(user.userId);

    // If no user found
    if (!userDetails) return ThrowUnAuthorizedError(res);
